...
import { PortfolioCategory, PortfolioProject } from '@/lib/portfolio-data';
...
function MetricCard({ label, value, detail, dark = false }: { label: string; value: string; detail: string; dark?: boolean }) {
  return (
    <div className={`rounded-[26px] border p-5 ${dark ? 'border-[rgba(219,219,219,0.2)] bg-[rgba(219,219,219,0.12)] text-white' : 'border-black/[0.06] bg-[rgba(255,255,255,0.52)] text-navy'}`}>
      ...
      <p className="mt-5 font-display text-5xl uppercase tracking-[0.12em]">{value}</p>
    ...
    <div className="mx-auto max-w-7xl px-4 pt-32 md:px-6 lg:pt-36">
      <section ref={heroRef} className={`section-shell overflow-hidden rounded-[42px] border border-black/[0.06] ${darkHero ? 'bg-[linear-gradient(135deg,#061B3A,#05070d)] text-white' : 'bg-[linear-gradient(135deg,#e7e7e7,#d8d8d8)] text-navy'} px-6 py-10 md:px-10 md:py-14`}>
        ...
        <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
          <div>
            ...
            <h1 data-animate className={`mt-4 max-w-4xl font-display text-[clamp(4rem,11vw,11rem)] uppercase leading-[0.88] tracking-[0.12em] ${darkHero ? 'text-[#F8F8F8] [text-shadow:0_4px_30px_rgba(0,0,0,0.12)]' : 'text-on-light'}`}>
              {project.title}
            </h1>
          ...
          <div className="mt-6 space-y-4">
            {project.process.map((step, index) => (
              <div key={step.title} className="rounded-[20px] border border-black/[0.06] bg-[linear-gradient(180deg,#e8e8e8,#d9d9d9)] px-5 py-5">
                <p className="font-display text-3xl uppercase tracking-[0.12em] text-orange">0{index + 1}</p>
                <h2 className="mt-4 font-display text-4xl uppercase tracking-[0.1em] text-navy">{step.title}</h2>
      ...
      <section className="section-shell mt-12 space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {project.visuals.map((visual, index) => (
            ...
            >
              <h3 className={`font-display text-4xl uppercase tracking-[0.1em] ${index % 2 === 0 ? 'text-on-light' : 'text-primary text-shadow-soft'}`}>{visual.title}</h3>
      ...
      <section className="section-shell mt-12 grid gap-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
        <div className="rounded-[34px] border border-black/[0.06] bg-[linear-gradient(180deg,rgba(255,255,255,0.72),rgba(255,255,255,0.5))] p-6 shadow-[0_24px_70px_rgba(0,0,0,0.1)] md:p-8">
          ...
          <blockquote className="mt-6 font-display text-5xl uppercase leading-[0.94] tracking-[0.1em] text-on-light md:text-6xl">
            “{project.testimonial.quote}”
          </blockquote>
          <p className="mt-6 text-sm uppercase tracking-[0.28em] text-black/72">{project.testimonial.author} • {project.testimonial.role}</p>
        </div>
        ...
        <div className="rounded-[34px] border border-[rgba(255,255,255,0.35)] bg-[linear-gradient(135deg,#061B3A,#07111f)] p-6 text-white shadow-[0_24px_70px_rgba(0,0,0,0.18)] md:p-8">
          ...
          <h2 className="mt-4 font-display text-5xl uppercase leading-[0.9] tracking-[0.12em] text-primary text-shadow-soft md:text-7xl">Let’s Build Your Brand</h2>
          ...
          <div className="mt-8 flex flex-wrap gap-4">
            <Link href="/contact" className="rounded-full bg-orange px-6 py-4 font-groove text-xs font-semibold uppercase tracking-[0.28em] text-dark">
              Book Strategy Call
            </Link>
            <Link href={category.href} className="rounded-full border border-[rgba(219,219,219,0.24)] bg-[rgba(219,219,219,0.12)] px-6 py-4 font-groove text-xs font-semibold uppercase tracking-[0.28em] text-white">
              Back to category
            </Link>
          </div>
        </div>
