...
export default function AboutPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 pt-32 md:px-6 lg:pt-36">
      ...
      <section className="section-shell mt-16 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          >
            ...
            <h2 className="mt-10 font-display text-4xl uppercase tracking-[0.12em] text-navy">{title}</h2>
            <p className="mt-4 text-sm leading-7 text-black/80">{copy}</p>
          </motion.article>
        ))}
      </section>

      <section className="section-shell mt-20 grid gap-10 lg:grid-cols-[0.92fr_1.08fr] lg:items-start">
        <div className="rounded-[36px] border border-black/[0.06] bg-[linear-gradient(180deg,#061B3A,#07111f)] p-6 text-white md:p-10">
          <SectionHeading
            eyebrow="Company Story"
            title="We care about the feeling before the funnel."
            description="A premium agency should look like it understands the brand before it starts explaining the work."
            dark
          />
          ...
          {milestones.map((item, index) => (
            ...
            >
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <p className="font-display text-5xl uppercase tracking-[0.12em] text-orange">{item.year}</p>
      ...
      <section className="section-shell mt-20 pb-24">
        <SectionHeading
          ...
          title="A premium look with a disciplined process."
          description="The work feels high-end because the system is disciplined: it balances whitespace, motion restraint, and strong visual hierarchy."
          align="center"
        />
      </section>
    </div>
