...
export default function PortfolioStrip() {
  return (
    <div className="overflow-hidden rounded-[36px] border border-[rgba(255,255,255,0.18)] bg-[linear-gradient(135deg,#07111F,#061B3A)] p-4 shadow-[0_28px_90px_rgba(0,0,0,0.14)] md:p-6">
      <motion.div className="flex gap-4 md:gap-6" animate={{ x: ['0%', '-55%'] }} transition={{ duration: 28, repeat: Infinity, ease: 'linear' }}>
        {[...cases, ...cases].map((item, index) => (
          <article key={`${item.title}-${index}`} className="group min-h-[320px] w-[78vw] shrink-0 rounded-[30px] p-5 text-white md:w-[48vw] lg:w-[34vw]">
            <div className="flex h-full flex-col justify-between overflow-hidden rounded-[26px] border border-[rgba(255,255,255,0.35)] bg-[linear-gradient(135deg,#061B3A,#05070d)] p-5 shadow-[0_25px_80px_rgba(0,0,0,0.12)] backdrop-blur-[6px] md:p-6">
              <div className="overflow-hidden rounded-[24px] border border-[rgba(255,255,255,0.35)] shadow-[0_16px_50px_rgba(0,0,0,0.14)]">
                <DynamicImage src={item.coverImage} alt={item.title} className="aspect-[16/10] w-full" imageClassName="transition-transform duration-700 group-hover:scale-105" />
              </div>
              ...
              <div>
                <p className="fluid-card-title font-display uppercase leading-[0.92] tracking-[0.08em] text-primary text-shadow-soft">{item.title}</p>
