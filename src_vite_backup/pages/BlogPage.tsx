...
const posts = [
  ...
  { title: 'How to build cinematic scroll flow in React', category: 'Motion', featured: false, copy: 'A practical breakdown of sticky sections, layered transitions, and pacing.', coverImage: 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?auto=format&fit=crop&w=1400&q=80' },
...
export default function BlogPage() {
  ...
  const filtered = useMemo(() => (activeCategory === 'All' ? posts : posts.filter((post) => post.category === activeCategory)), [activeCategory]);
  ...
  return (
    <div className="container-laptop mx-auto px-4 pt-36 md:px-6 md:pt-40 lg:pt-44">
      ...
      <section className="section-shell mt-10 grid gap-6 lg:grid-cols-[1.1fr_0.9fr] lg:mt-12">
        <motion.article initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} className="rounded-[36px] border border-black/[0.06] bg-[linear-gradient(135deg,#061B3A,#08101f)] p-6 text-white md:p-10">
          <p className="font-groove text-[11px] uppercase tracking-[0.38em] text-white/86">Featured article</p>
          <h2 className="mt-4 max-w-xl fluid-card-title font-display uppercase leading-[0.92] tracking-[0.08em]">{featured.title}</h2>
        ...
        <div className="grid gap-4">
          {filtered.map((post, index) => (
            ...
            >
              ...
              <h3 className="mt-7 fluid-card-title font-display uppercase leading-[0.92] tracking-[0.08em] text-navy">{post.title}</h3>
